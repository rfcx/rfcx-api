  ...
  .get(passport.authenticate(['jwt', 'jwt-custom'], { session: false }), hasRole(['rfcxUser']), function (req, res) {
    ...
    params.validate()
      ...
      .then((dbStream) => {
        streamsService.checkUserAccessToStream(req, dbStream)
        stream = dbStream
        const starts = transformedParams.starts
        const ends = transformedParams.ends
    ...
    params.validate()
      ...
      .then((segmentFormatted) => {
        return streamsService.refreshStreamStartEnd(stream)
          .then(() => {
            const starts = moment.tz(transformedParams.starts, 'UTC').startOf('day').valueOf()
            const ends = moment.tz(transformedParams.ends, 'UTC').endOf('day').valueOf()
  ...
  .get(passport.authenticate(['jwt', 'jwt-custom'], { session: false }), hasRole(['rfcxUser', 'systemUser']), function (req, res) {
    ...
    params.validate()
      ...
      .then((dbStream) => {
        ...
        const starts = transformedParams.starts ? transformedParams.starts : dbStream.starts
        const ends = transformedParams.ends ? transformedParams.ends : dbStream.ends
        const limit = transformedParams.limit
        const offset = transformedParams.offset
