...
function createUserWithParams (user) {
  const query = 'CREATE (user:user { guid: {guid}, firstname: {firstname}, lastname: {lastname}, email: {email}, username: {username}, pictureUrl: {pictureUrl} }) RETURN user'
...
function updateUserPicture (user) {
  const query = 'MATCH (user:user { guid: {guid}, email: {email} }) SET user.pictureUrl = {pictureUrl} RETURN user'
