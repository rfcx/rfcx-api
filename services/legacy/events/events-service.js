...
function prepareOpts (req) {
  ...
  const opts = {
    ...
    order: order || 'GuardianAudioEvent.begins_at',
    dir: dir || 'ASC'
  }
  ...
  return prepareOpts(req)
    .then((opts) => {
      ...
      return models.sequelize
        ...
        .then((events) => {
          const evs = filterEventsWithTz(opts, events)
          return evs.length
        })
    })
}
...
function filterEventsWithTz (opts, events) {
  return events.filter((event) => {
    const beginsAtTz = moment.tz(event.begins_at, event.site_timezone)
    const endsAtTz = moment.tz(event.ends_at, event.site_timezone)
