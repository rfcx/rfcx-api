...
let streamBaseInclude = [
  {
    ...
    as: 'location',
    attributes: models.Location.attributes.lite
  },
  {
...
function create(data, joinRelations = false) {
  ...
  const { id, name, description, start, end, is_private, location_id, created_by_id } = data; // do not use raw input object for security reasons
  return models.Stream
...
function update(stream, data, joinRelations = false) {
  ['name', 'description', 'is_private', 'start', 'end', 'location_id'].forEach((attr) => {
...
function formatStream(stream) {
  const { id, name, description, start, end, is_private, created_at, updated_at } = stream;
  return {
    id,
    name,
    description,
