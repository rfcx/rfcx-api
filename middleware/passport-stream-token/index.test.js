...
const streamServicePath = '../../services/streams/index'
...
describe('stream-token strategy', () => {
  ...
  test('Should call callback function with super user + system user flags', async () => {
    ...
    expect(mockCallback.mock.calls.length).toBe(1)
    expect(mockCallback.mock.calls[0][0]).toBeNull()
    expect(mockCallback.mock.calls[0][1].is_super).toBeTruthy()
    expect(mockCallback.mock.calls[0][1].has_system_role).toBeTruthy()
  })
})
