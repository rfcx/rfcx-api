...
router.route('/:guid/sns-sqs')
  .get(passport.authenticate(['token', 'jwt'], { session: false }), hasRole(['aiAdmin']), function (req, res) {
    return aiService.getPublicAiByGuid(req.params.guid)
      ...
      .catch(ValidationError, e => httpErrorResponse(req, res, 400, null, e.message))
      .catch(EmptyResultError, e => httpErrorResponse(req, res, 404, null, e.message))
      .catch(e => { httpErrorResponse(req, res, 500, e, `Error while getting SNS topic and SQS queue for AI with guid "${req.params.guid}".`); console.error(e) })
  })
...
router.route('/:guid/sns-sqs')
  .post(passport.authenticate(['token', 'jwt'], { session: false }), hasRole(['aiAdmin']), function (req, res) {
    return aiService.getPublicAiByGuid(req.params.guid)
      ...
      .catch(ValidationError, e => httpErrorResponse(req, res, 400, null, e.message))
      .catch(EmptyResultError, e => httpErrorResponse(req, res, 404, null, e.message))
      .catch(e => { httpErrorResponse(req, res, 500, e, `Error while creating SNS topic and SQS queue for AI with guid "${req.params.guid}".`); console.error(e) })
  })
...
router.route('/:guid/batch-analysis')
  .get(passport.authenticate(['token', 'jwt'], { session: false }), hasRole(['aiAdmin']), function (req, res) {
    ...
    return aiService.getPublicAiByGuid(req.params.guid)
      ...
      .catch(ValidationError, e => httpErrorResponse(req, res, 400, null, e.message))
      .catch(EmptyResultError, e => httpErrorResponse(req, res, 404, null, e.message))
      .catch(e => { httpErrorResponse(req, res, 500, e, 'Error while triggering batch analysis.'); console.error(e) })
  })
...
router.route('/:guid/download')
  .get(passport.authenticate(['token', 'jwt'], { session: false }), hasRole(['rfcxUser']), function (req, res) {
    ...
    return dirUtil.ensureDirExists(opts.filePath)
      ...
      .catch(EmptyResultError, e => httpErrorResponse(req, res, 404, null, e.message))
      .catch(e => { httpErrorResponse(req, res, 500, e, 'Error while downloading for model.'); console.error(e) })
  })
