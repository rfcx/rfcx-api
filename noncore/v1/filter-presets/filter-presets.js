...
router.route('/')
  .post(passport.authenticate(['token', 'jwt', 'jwt-custom'], { session: false }), hasRole(['rfcxUser']), function (req, res) {
    ...
    filterPresetsService.doesNameExist(serviceParams.name)
      ...
      .catch(e => { console.log('e', e); httpErrorResponse(req, res, 500, e, "Filter preset couldn't be created.") })
  })
...
router.route('/:guid')
  .post(passport.authenticate(['token', 'jwt', 'jwt-custom'], { session: false }), hasRole(['rfcxUser']), function (req, res) {
    ...
    usersService.getUserByGuid(req.rfcx.auth_token_info.guid)
      ...
      .catch(sequelize.EmptyResultError, e => httpErrorResponse(req, res, 404, null, e.message))
      .catch(ValidationError, e => httpErrorResponse(req, res, 400, null, e.message))
      .catch(e => { console.log('e', e); httpErrorResponse(req, res, 500, e, "Filter preset couldn't be updated.") })
  })
...
router.route('/:guid')
  .get(passport.authenticate(['token', 'jwt', 'jwt-custom'], { session: false }), hasRole(['rfcxUser']), function (req, res) {
    filterPresetsService.getFilterPresetByGuid(req.params.guid)
      ...
      .catch(sequelize.EmptyResultError, e => httpErrorResponse(req, res, 404, null, e.message))
      .catch(ValidationError, e => httpErrorResponse(req, res, 400, null, e.message))
      .catch(e => { console.log('e', e); httpErrorResponse(req, res, 500, e, "Can't get filter preset") })
  })
...
router.route('/')
  .get(passport.authenticate(['token', 'jwt', 'jwt-custom'], { session: false }), hasRole(['rfcxUser']), function (req, res) {
    ...
    filterPresetsService.getFilterPresets(serviceParams)
      ...
      .catch(sequelize.EmptyResultError, e => httpErrorResponse(req, res, 404, null, e.message))
      .catch(ValidationError, e => httpErrorResponse(req, res, 400, null, e.message))
      .catch(e => { console.log('e', e); httpErrorResponse(req, res, 500, e, "Can't get filter presets") })
  })
