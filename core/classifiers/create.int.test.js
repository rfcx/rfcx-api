...
const CLASSIFICATION_3 = { id: 3, value: 'chainsaw_2', title: 'chainsaw 2', typeId: 1, sourceId: null, parent_id: null, source_external_id: null, created_at: '2022-06-29 11:22:37.094935' }
...
describe('POST /classifiers/:id', () => {
  ...
  describe('valid usage', () => {
    ...
    test('map from:to:threshold correct classification value (with underscore _) and correct threshold', async () => {
      // Arrange
      const classifierLabel = 'chain_saw'
      const dbLabel = CLASSIFICATION_3.value
      const threshold = 0.2
      const requestBody = {
        name: 'chainsaw',
        version: 1,
      ...
      const classifierOutputs = await models.ClassifierOutput.findAll()
      expect(classifierOutputs.length).toBe(1)
      expect(classifierOutputs[0].classificationId).toBe(CLASSIFICATION_3.id)
      expect(classifierOutputs[0].outputClassName).toBe(classifierLabel)
      expect(classifierOutputs[0].ignoreThreshold).toBe(threshold)
    })
    ...
    test('map to:threshold correct classification value (with underscore _) and correct threshold', async () => {
      // Arrange
      const dbLabel = CLASSIFICATION_3.value
      const threshold = 0.2
      const requestBody = {
    ...
    test('multiple only to classification values (with underscore _)', async () => {
      // Arrange
      const dbLabel1 = CLASSIFICATION_1.value
      const dbLabel2 = CLASSIFICATION_2.value
      const dbLabel3 = CLASSIFICATION_3.value
      const requestBody = {
        name: 'chainsaw_and_vehicle_and_chainsaw_2',
        version: 1,
        classification_values: [`${dbLabel1}`, `${dbLabel2}`, `${dbLabel3}`]
      }
      ...
      expect(response.statusCode).toBe(201)
      const classifierOutputs = await models.ClassifierOutput.findAll()
      expect(classifierOutputs.length).toBe(3)
      expect(classifierOutputs[0].classificationId).toBe(CLASSIFICATION_1.id)
      expect(classifierOutputs[0].outputClassName).toBe(dbLabel1)
      expect(classifierOutputs[0].ignoreThreshold).toBe(0.5)
      expect(classifierOutputs[1].classificationId).toBe(CLASSIFICATION_2.id)
      expect(classifierOutputs[1].outputClassName).toBe(dbLabel2)
      expect(classifierOutputs[1].ignoreThreshold).toBe(0.5)
      expect(classifierOutputs[2].classificationId).toBe(CLASSIFICATION_3.id)
      expect(classifierOutputs[2].outputClassName).toBe(dbLabel3)
      expect(classifierOutputs[2].ignoreThreshold).toBe(0.5)
    })
  })
  ...
  describe('invalid usage', () => {
    ...
    test('400 from:to:threshold if 1 of classification not exist in db', async () => {
      // Arrange
      const classifierLabel1 = 'chain'
      const dbLabel1 = CLASSIFICATION_1.value
      const threshold1 = 0.1
      const requestBody = {
        name: 'abc',
        version: 1,
        classification_values: [`${classifierLabel1}:${dbLabel1}:${threshold1}`, 'abc:def:0.5']
      }
    ...
    test('400 classification_values is invalid format (threshold out of range > max)', async () => {
      // Arrange
      const requestBody = {
        name: 'abc',
        version: 1,
        classification_values: 'chainsaw:chainsaw:1.01'
      }
    ...
    test('400 classification_values is invalid format (threshold out of range < min)', async () => {
      // Arrange
      const requestBody = {
        name: 'abc',
        version: 1,
        classification_values: 'chainsaw:chainsaw:-0.01'
      }
    ...
    test('400 classification_values is invalid format (threshold decimal places > 2)', async () => {
      // Arrange
      const requestBody = {
        name: 'abc',
        version: 1,
        classification_values: 'chainsaw:chainsaw:0.903'
      }
