...
describe('GET /classifier-jobs/{id}/validation', () => {
  ...
  describe('Invalid', () => {
    test('returns forbidden error', async () => {
      const response = await request(app).get(`/${JOB_5.id}/validation`)
      expect(response.statusCode).toBe(403)
    })

    test('returns empty error', async () => {
      const response = await request(app).get('/230000123/validation')
