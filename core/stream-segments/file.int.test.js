...
describe('GET /streams/:id/segments/:start/file', () => {
  test('segment found', async () => {
    ...
    const segment = await models.StreamSegment.create({ stream_id: stream.id, path: `2021/07/26/${stream.id}`, start: '2021-07-26T10:10:10.000Z', end: '2021-07-26T10:11:09.999Z', stream_source_file_id: sourceFile.id, sample_count: 720000, file_extension_id: 1, availability: 1 })
  ...
  test('segment path is obtained from path column when path and start dont match by date', async () => {
    ...
    await models.UserStreamRole.create({ stream_id: stream.id, user_id: stream.createdById, role_id: seedValues.roleOwner })
    const sourceFile = await models.StreamSourceFile.create({ stream_id: stream.id, filename: '20210726_101010.wav', duration: 60, sample_count: 720000, sample_rate: 12000, channels_count: 1, bit_rate: 1, audio_codec_id: 1, audio_file_format_id: 1 })
    const segment = await models.StreamSegment.create({ stream_id: stream.id, path: `2021/07/24/${stream.id}`, start: '2021-07-26T10:10:10.000Z', end: '2021-07-26T10:11:09.999Z', stream_source_file_id: sourceFile.id, sample_count: 720000, file_extension_id: 1, availability: 1 })
